...
exports.PORT = exports.HOST = exports.MaxSocketTransport = void 0;
exports.packPacket = packPacket;
exports.unpackPacket = unpackPacket;
const tls_1 = __importDefault(require("tls"));
const msgpack_1 = require("@msgpack/msgpack");
const binding_js_1 = require("lz4/lib/binding.js");
...
function packPacket(ver, cmd, seq, opcode, payload) {
    const payloadBuf = payload ? Buffer.from((0, msgpack_1.encode)(payload)) : Buffer.alloc(0);
    ...
    if (payloadLength > 0 && data.length >= 10 + payloadLength) {
        ...
        try {
            if (compFlag !== 0) {
                ...
                const n = (0, binding_js_1.uncompress)(payloadBytes, out);
                if (n > 0)
                    payload = (0, msgpack_1.decode)(out.subarray(0, n));
            }
            else {
                payload = (0, msgpack_1.decode)(payloadBytes);
            }
        }
        catch {
            payload = null;
...
function readExactlyFromBuffer(transport, n) {
    return new Promise((resolve) => {
        ...
        const onData = () => {
            if (tryResolve())
                transport.socket?.removeListener('data', onData);
        };
...
class MaxSocketTransport {
    constructor(options = {}) {
        ...
        this.deviceId = options.deviceId || (0, uuid_1.v4)();
    ...
    connect() {
        return new Promise((resolve, reject) => {
            ...
            const sock = tls_1.default.connect(opts, () => {
                this.socket = sock;
                this._recvBuffer = Buffer.alloc(0);
                sock.on('data', (chunk) => {
    ...
    async sendAndWait(opcode, payload, cmd = 0, timeout = 20000) {
        ...
        const promise = new Promise((resolve, reject) => {
            const t = setTimeout(() => {
                ...
                if (p) {
                    this.pending.delete(seqKey);
                    p.reject(new Error(`Timeout waiting for opcode ${(0, opcodes_1.getOpcodeName)(opcode)}`));
                }
            }, timeout);
            pendingRef = {
    ...
    async handshake(userAgentPayload) {
        const ua = userAgentPayload || new userAgent_1.UserAgentPayload({
        ...
        const uaJson = ua instanceof userAgent_1.UserAgentPayload ? ua.toJSON() : ua;
        const payload = {
            deviceId: this.deviceId,
            userAgent: uaJson
        };
        const resp = await this.sendAndWait(opcodes_1.Opcode.SESSION_INIT, payload);
        this._log('Handshake OK');
        return resp;
    }
    async requestCode(phone, language = 'ru') {
        const payload = { phone, type: 'START_AUTH', language };
        const data = await this.sendAndWait(opcodes_1.Opcode.AUTH_REQUEST, payload);
    ...
    async sendCode(tempToken, code) {
        ...
        const data = await this.sendAndWait(opcodes_1.Opcode.AUTH, payload);
        return data.payload;
    }
    async sync(token, userAgentJson) {
        ...
        if (userAgentJson)
            payload.userAgent = userAgentJson;
        const data = await this.sendAndWait(opcodes_1.Opcode.LOGIN, payload);
        return data.payload;
    }
    async getChats(marker = 0) {
        const data = await this.sendAndWait(opcodes_1.Opcode.CHATS_LIST, { marker });
        return (data.payload?.chats) || [];
    }
    async getHistory(chatId, from = Date.now(), backward = 200, forward = 0) {
        const data = await this.sendAndWait(opcodes_1.Opcode.CHAT_HISTORY, {
            chatId,
            from,
            forward,
            backward,
...
exports.MaxSocketTransport = MaxSocketTransport;
