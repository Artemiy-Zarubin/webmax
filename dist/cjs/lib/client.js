...
const socketTransport_1 = require("./socketTransport");
...
function loadSessionConfig(configPath) {
    ...
    if (path_1.default.isAbsolute(configPath)) {
        ...
        resolved = path_1.default.join(process.cwd(), 'config', `${configPath}.json`);
    }
    else {
        resolved = path_1.default.join(process.cwd(), configPath);
    }
    if (!fs_1.default.existsSync(resolved)) {
        throw new Error(`Конфиг не найден: ${resolved}`);
    }
    const data = fs_1.default.readFileSync(resolved, 'utf8');
 ...
 */
class WebMaxClient extends events_1.default {
    constructor(options = {}) {
        super();
        ...
        this.session = new session_1.SessionManager(this.sessionName);
        const userAgentFromOptions = options.userAgent;
        const resolvedUserAgentOptions = userAgentFromOptions
            ? (userAgentFromOptions instanceof userAgent_1.UserAgentPayload ? userAgentFromOptions.toJSON() : userAgentFromOptions)
            : null;
        ...
        this._handshakeUserAgent = new userAgent_1.UserAgentPayload(webDefaults);
        this.userAgent = this._handshakeUserAgent;
        this.deviceId = options.deviceId || this.session.get('deviceId') || (0, uuid_1.v4)();
        ...
        this.ver = 11;
        this.handlers = {
            [constants_1.EventTypes.START]: [],
            [constants_1.EventTypes.MESSAGE]: [],
            [constants_1.EventTypes.MESSAGE_REMOVED]: [],
            [constants_1.EventTypes.CHAT_ACTION]: [],
            [constants_1.EventTypes.ERROR]: [],
            [constants_1.EventTypes.DISCONNECT]: []
        };
        this.messageQueue = [];
    ...
    onStart(handler) {
        if (typeof handler === 'function') {
            this.handlers[constants_1.EventTypes.START].push(handler);
            return handler;
        }
        return (fn) => {
            this.handlers[constants_1.EventTypes.START].push(fn);
            return fn;
        };
    }
    /**
    ...
    onMessage(handler) {
        if (typeof handler === 'function') {
            this.handlers[constants_1.EventTypes.MESSAGE].push(handler);
            return handler;
        }
        return (fn) => {
            this.handlers[constants_1.EventTypes.MESSAGE].push(fn);
            return fn;
        };
    }
    /**
    ...
    onMessageRemoved(handler) {
        if (typeof handler === 'function') {
            this.handlers[constants_1.EventTypes.MESSAGE_REMOVED].push(handler);
            return handler;
        }
        return (fn) => {
            this.handlers[constants_1.EventTypes.MESSAGE_REMOVED].push(fn);
            return fn;
        };
    }
    /**
    ...
    onChatAction(handler) {
        if (typeof handler === 'function') {
            this.handlers[constants_1.EventTypes.CHAT_ACTION].push(handler);
            return handler;
        }
        return (fn) => {
            this.handlers[constants_1.EventTypes.CHAT_ACTION].push(fn);
            return fn;
        };
    }
    /**
    ...
    onError(handler) {
        if (typeof handler === 'function') {
            this.handlers[constants_1.EventTypes.ERROR].push(handler);
            return handler;
        }
        return (fn) => {
            this.handlers[constants_1.EventTypes.ERROR].push(fn);
            return fn;
        };
    }
    /**
    ...
    async start() {
        try {
            ...
            await this.triggerHandlers(constants_1.EventTypes.START);
            console.log('\n✅ Клиент запущен успешно!');
        }
        catch (error) {
            console.error('❌ Ошибка при запуске клиента:', error);
            await this.triggerHandlers(constants_1.EventTypes.ERROR, error);
            throw error;
        }
    }
    /**
    ...
    async requestQR() {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.GET_QR, {});
    ...
    async checkQRStatus(trackId) {
        const response = await this.sendAndWait(opcodes_1.Opcode.GET_QR_STATUS, { trackId });
        if (response.payload && response.payload.error) {
    ...
    async loginByQR(trackId) {
        const response = await this.sendAndWait(opcodes_1.Opcode.LOGIN_BY_QR, { trackId });
        if (response.payload && response.payload.error) {
    ...
    async sync() {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.LOGIN, payload);
        if (response.payload && response.payload.error) {
        ...
        if (responsePayload.profile && responsePayload.profile.contact) {
            ...
            this.me = new entities_1.User(userData);
    ...
    async fetchMyProfile() {
        try {
            ...
            const response = await this.sendAndWait(opcodes_1.Opcode.PROFILE, {});
            const payload = response.payload;
            if (payload && payload.user) {
                this.me = new entities_1.User(payload.user);
    ...
    async _connectSocket() {
        ...
        this._socketTransport = new socketTransport_1.MaxSocketTransport({
    ...
    async _connectWebSocket() {
        ...
        return new Promise((resolve, reject) => {
            ...
            this.ws.on('error', (error) => {
                ...
                this.triggerHandlers(constants_1.EventTypes.ERROR, error);
                reject(error);
            });
            this.ws.on('close', () => {
                ...
                this.triggerHandlers(constants_1.EventTypes.DISCONNECT);
    ...
    async handshake() {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.SESSION_INIT, payload);
        if (response.payload && response.payload.error) {
    ...
    async handleSocketNotification(data) {
        try {
            if (this.debug && data.opcode !== opcodes_1.Opcode.PING) {
                const payload = data.payload?.error ? ` error=${JSON.stringify(data.payload.error)}` : '';
                console.log(`📥 ${(0, opcodes_1.getOpcodeName)(data.opcode)} (seq=${data.seq})${payload}`);
            }
            switch (data.opcode) {
                case opcodes_1.Opcode.NOTIF_MESSAGE:
                    await this.handleNewMessage(data.payload);
                    break;
                case opcodes_1.Opcode.NOTIF_MSG_DELETE:
                    await this.handleRemovedMessage(data.payload);
                    break;
                case opcodes_1.Opcode.NOTIF_CHAT:
                    await this.handleChatAction(data.payload);
                    break;
                case opcodes_1.Opcode.PING:
                    break;
                default:
    ...
    async handleMessage(data) {
        try {
            ...
            if (this.debug && message.opcode !== opcodes_1.Opcode.PING) {
                const payload = message.payload?.error ? ` error=${JSON.stringify(message.payload.error)}` : '';
                console.log(`📥 ${(0, opcodes_1.getOpcodeName)(message.opcode || 0)} (seq=${message.seq})${payload}`);
            }
    ...
    async sendPong() {
        try {
            const message = this.makeMessage(opcodes_1.Opcode.PING, {});
    ...
    async handleNewMessage(data) {
        ...
        const message = new entities_1.Message(messageData, this);
        if (!message.sender && message.senderId && message.senderId !== this.me?.id) {
            await message.fetchSender();
        }
        await this.triggerHandlers(constants_1.EventTypes.MESSAGE, message);
    }
    /**
     * Обработка удаленного сообщения
     */
    async handleRemovedMessage(data) {
        const message = new entities_1.Message(data, this);
        await this.triggerHandlers(constants_1.EventTypes.MESSAGE_REMOVED, message);
    }
    /**
     * Обработка действия в чате
     */
    async handleChatAction(data) {
        const action = new entities_1.ChatAction(data, this);
        await this.triggerHandlers(constants_1.EventTypes.CHAT_ACTION, action);
    }
    /**
    ...
    async sendMessage(options) {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.MSG_SEND, payload);
        if (response.payload && response.payload.message) {
            return new entities_1.Message(response.payload.message, this);
        }
        return response.payload;
    }
    /**
    ...
    async editMessage(options) {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.MSG_EDIT, payload);
        if (response.payload && response.payload.message) {
            return new entities_1.Message(response.payload.message, this);
        }
        return response.payload;
    ...
    async deleteMessage(options) {
        ...
        await this.sendAndWait(opcodes_1.Opcode.MSG_DELETE, payload);
        return true;
    }
    /**
    ...
    async getUser(userId) {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.CONTACT_INFO, payload);
        if (response.payload && response.payload.contacts && response.payload.contacts?.length) {
            ...
            return new entities_1.User(userData);
        }
        return null;
    }
    /**
    ...
    async getChats(marker = 0) {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.CHATS_LIST, payload);
    ...
    async getHistory(chatId, from = Date.now(), backward = 200, forward = 0) {
        if (this._useSocketTransport && this._socketTransport) {
            ...
            return messages.map(msg => new entities_1.Message(msg, this));
        }
        const payload = {
            chatId: chatId,
            from: from,
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.CHAT_HISTORY, payload);
    ...
    async getFileLink(options) {
        ...
        const response = await this.sendAndWait(opcodes_1.Opcode.FILE_DOWNLOAD, { fileId, chatId, messageId });
        const payload = response.payload || {};
        if (payload.error) {
...
exports.WebMaxClient = WebMaxClient;
